...
class DaurohController extends Controller
{
	/**
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Dauroh']))
		{
			$model->attributes=$_POST['Dauroh'];
			$model->title_slug=strtolower(str_replace(" ", "-", $_POST['Dauroh']['judul']));
			// get image uploaded
			...
			if($model->save())
				...
				for ($i = 0; $i < count($file_name); $i++) {
					if ($file_name[$i] != "") {
						$model_gallery[$i]=new DaurohGallery;
						$model_gallery[$i]->gambar_id=$gambar_id;
						$model_gallery[$i]->gambar=$file_name[$i];
						if ($model_gallery[$i]->save())
							move_uploaded_file($file_tmp[$i], "./images/dauroh_gallery/".$file_name[$i]);
					}
				}
	...
	{
		//$dataProvider=new CActiveDataProvider('Dauroh');
		$model=new Dauroh('search');
		$model->unsetAttributes(); // clear default value
		// $this->render('index',array(
		// 	'dataProvider'=>$dataProvider,
		// ));
		if (isset($_GET['Dauroh']))
			$model->attributes=$_GET['Dauroh'];
		$this->render('index',array(
			//'dataProvider'=>$dataProvider,
	 ...
	 * @return Dauroh the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=Dauroh::model()->findByPk($id);
		if($model===null)
	...
	{
		$model=Dauroh::model()->find('title_slug=:title_slug', array(':title_slug'=>$title_slug));
		if($model===null)
			throw new CHttpException(404,'The requested page does not exist.');
		return $model;
	}
	 ...
	 * @param Dauroh $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='dauroh-form')
		{
