...
class UstadController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Ustad']))
		{
			$model->attributes=$_POST['Ustad'];
			$uploadedFile=CUploadedFile::getInstance($model, 'foto');
			$model->foto = $uploadedFile;
			if($model->save())
				$uploadedFile->saveAs(Yii::app()->basePath.'/../images/ustad/'.$uploadedFile);
	...
	{
		$dataProvider=new CActiveDataProvider('Ustad');
	...
	{
		$model=new Ustad('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Ustad']))
			$model->attributes=$_GET['Ustad'];
	 ...
	 * @return Ustad the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=Ustad::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param Ustad $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='ustad-form')
		{
